...
export default async function HomePage() {
  ...
  return (
    <div className="hero-glow min-h-screen px-3 py-5 sm:px-6 sm:py-8 lg:px-8">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 sm:gap-6">

        {/* ── Hero banner ────────────────────────────────────────── */}
        <div className="card-shadow-lg relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 sm:rounded-3xl dark:border-slate-800 dark:bg-slate-900 sm:p-8 lg:p-10">
          ...
          <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 text-xs leading-6 text-slate-700 sm:text-sm sm:leading-7 dark:text-slate-400">
            ...
            <Link
              href="/issues/new"
              id="hero-create-btn"
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-linear-to-r from-cyan-500 to-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-cyan-500/25 transition-all hover:shadow-cyan-500/40 hover:brightness-110 sm:w-fit focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 dark:shadow-cyan-600/20"
            >
              + New Issue
            </Link>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className={`${stat.bg} card-shadow rounded-2xl border border-slate-200/80 p-3.5 sm:p-5 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_2px_4px_rgba(15,23,42,0.05),0_14px_34px_-10px_rgba(15,23,42,0.16)] dark:border-slate-800 dark:shadow-none dark:hover:shadow-none`}
            >
              <div className="flex items-center justify-between">
                <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-700 dark:text-slate-400 sm:text-xs">
                  {stat.label}
                </p>
                <span className="text-base sm:text-xl">{stat.icon}</span>
              </div>
              <p className="mt-1 text-2xl font-bold tabular-nums text-slate-900 sm:mt-2 sm:text-3xl lg:text-4xl dark:text-slate-50">
                {stat.value}
              </p>
              {/* Mini progress bar */}
              <div className="mt-2.5 h-1.5 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700 sm:mt-3">
                <div
                  className={`${stat.barColor} h-full rounded-full transition-all duration-700`}
                  style={{ width: `${stat.percent}%` }}
                />
              </div>
              <p className="mt-1 text-right text-[10px] text-slate-600 dark:text-slate-500 sm:text-xs">
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_340px] xl:grid-cols-[1fr_380px] lg:gap-6">
          ...
          {/* Recent Issues */}
          <div className="card-shadow rounded-2xl border border-slate-200 bg-white p-5 sm:rounded-3xl sm:p-6 dark:border-slate-800 dark:bg-slate-900">
            <div className="mb-4 flex items-center justify-between sm:mb-5">
              ...
              <Link
                ...
                className="rounded-lg px-2.5 py-1 text-xs font-medium text-cyan-600 transition-colors hover:bg-cyan-50 hover:text-cyan-700 dark:text-cyan-400 dark:hover:bg-cyan-950/30"
              >
                View all →
              </Link>
            </div>
            ...
            <div className="card-shadow rounded-2xl border border-slate-200 bg-white p-5 sm:rounded-3xl sm:p-6 dark:border-slate-800 dark:bg-slate-900">
              ...
              <div className="flex flex-col gap-2.5">
                <Link
                  ...
                  className="group flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50/70 p-3.5 transition-all hover:border-cyan-300 hover:bg-cyan-50/40 hover:shadow-sm dark:border-slate-800/60 dark:bg-slate-800/40 dark:hover:border-cyan-600 dark:hover:bg-cyan-950/30"
                >
                ...
                <Link
                  ...
                  className="group flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50/70 p-3.5 transition-all hover:border-violet-300 hover:bg-violet-50/40 hover:shadow-sm dark:border-slate-800/60 dark:bg-slate-800/40 dark:hover:border-violet-600 dark:hover:bg-violet-950/30"
                >
